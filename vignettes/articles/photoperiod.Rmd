...
data |> 
  ...
  head(3) |> 
  gt(caption = "Photoperiod Information for the first three observations") |> 
...
data |> 
  ...
  #from here on out is just formatting
  head(3) |> 
  gt(caption = "Photoperiod Information for the first three days") |> 
  fmt_duration("photoperiod") |> 
  fmt_datetime(c("dawn", "dusk"))
```
...
photoperiod(coordinates, dates, timezone) |> 
  #from here on out is just formatting
  gt(caption = "Photoperiod information for winter and summer solstice") |> 
  fmt_datetime(c("dawn", "dusk")) |> 
  fmt_duration("photoperiod")
```
...
photoperiod(coordinates, dates, timezone, solarDep = 0) |> 
  #from here on out is just formatting
  gt(caption = "Photoperiod information for sunrise/sunset") |> 
  fmt_datetime(c("dawn", "dusk")) |> 
  fmt_duration("photoperiod")
```
...
Adding photoperiod data to a dataset means it can be manipulated the same way any other column in the dataset. `aggregate_Date()` is a great way to condense multiple days of data collection into a single day. For this example, we will use the full dataset again to start off.

```{r aggregate_Date, fig.height = 8}
data |> 
  filter(Id %in% c(201, 205, 212, 216, 219, 222)) |> 
...
```{r vis solar depression, fig.height = 6}
...
multiple_solarDep |> 
  ...
  labs(title = "Differences in photoperiod depending on solar depression angle")
```

The influence of solar depression angle can also be displayed in a single plot:

```{r vis solar depression 2}
...
We will go through the example for `intensity of daytime light exposure`, but it works the same way for evening light exposure. Because our datasets start in the middle of the first day, and end in the middle of the last day, we will remove those days from the further analysis:
...
timing_light_exposure |> head(4) |> gt()
```

This step provided us with the first timing of light above 50 lux, and, incidentally, also the last timing of light above 50 lux, which we would need for the intensity of evening light. In the next step, we will add this information to our dataset. 
...
data |> 
  head(4) |> 
  select(Id, Datetime, MEDI, first_timing_above_50, dusk) |> 
  gt()
```

This provides us with an expanded dataset containing the light exposure time series, and information for dusk and first timing above 50 lux. By checking whether each observation is within the first timing of light and dusk, we prepare our data for the final step.
